...
//pre: true
//post: it fil up a list of list (pathes) where every list is a path to the value.
void fast_pathes(const T &value, list<list<T>> &pathes, list<T> &path,const BinaryTree &t) const{


  if(not t.isEmpty()){
    path.push_back(t.getRoot());
    if(t.getRoot()==value){
      pathes.push_back(path);
    }
    fast_pathes(value, pathes, path, t.getLeft());
    //H.I: it add into pathes the rute of all nodes equal to value in the left tree
    //FF: the number of nodes for evaluate decrease
    fast_pathes(value, pathes, path, t.getRight());
    //H.I: it add into pathes the rute of all nodes equal to value in the left tree
    //FF: the number of nodes for evaluate decrease
    path.pop_back();
  }
...
//pre: true
//post: it returns a list of list where every list is a path to the element value in the BT.
list<list<T>>pathes(const T &value) const{

  list<T> path;
  list<list<T>> pathes;
  fast_pathes(value,pathes,path,*this);
  return pathes;

}

//pre: true.
//post: it returns the value of a node that is the last node in common in the pathes between firstNode and secondNode.
T search_lower_common_father(T firstNode, T secondNode)const{
  list<list<T>> l1=pathes(firstNode);
  list<list<T>> l2 =pathes(secondNode);

  typename list<list<T>>::iterator itl1=l1.begin();
  typename list<list<T>>::iterator itl2=l2.begin();



  int profundidad=0;
  T lower_common_father="";

  while(itl1!=l1.end()){
    //INV: it travels in every path of the first list
    while(itl2!=l2.end()){
      //INV: it travels in every path of the second list
      typename list<T>::iterator itpathl1=(*itl1).begin();
      typename list<T>::iterator itpathl2=(*itl2).begin();
      bool found=false;
      int profundidad_actual=0;
      T actual_lower_common_father;
      while(not found and itpathl1!=(*itl1).end() and itpathl2!=(*itl2).end()){
        //INV: it go trought every step in each path while they're the equal.
        if((*itpathl1)!=(*itpathl2)){
          found=true;
        }else{
          actual_lower_common_father=(*itpathl1);
        }
        ++itpathl1;
        ++itpathl2;
        ++profundidad_actual;
       ...
       if(profundidad_actual>profundidad){
         lower_common_father=actual_lower_common_father;
  ...
  return lower_common_father;
}

//Pre: true.
//Post: it returns true if the value is in the BT.
bool search(T value)const{
  bool there_is = false;

  if(not isEmpty() and not there_is){
    if(value == root){
      there_is = true;
    }else{
      there_is = (*left).search(value);
      //H.I: it returns true if the value is in the left BT.
      //Fita: the size of the tree decrease
      if(not there_is){
        there_is = (*right).search(value);
        //H.I: it returns true if the value is in the right BT
        //Fita: the size of the tree decrease
      }
    }
  }
